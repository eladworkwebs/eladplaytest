...
const Contact = () => {
  const contactInfo = [{
    ...
    title: "Direcciones",
    details: ["Andador del Carmen #11B, Avenida Miguel Hidalgo", "Andador de Guadalupe #5C, Belisario Domínguez"],
    action: {
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
            {contactInfo.map((info, index) => <div key={index} className="p-6 bg-white rounded-lg shadow-md border border-gray-200 flex flex-col">
                ...
                <div className="space-y-2 mb-6">
                  {info.details.map((detail, i) => <p key={i} className="text-gray-700">
                      {detail}
                    </p>)}
                </div>
                <div className="mt-auto">
                  {info.action.isLink ? <Button asChild variant="outline" className="w-full border-brand-blue text-brand-blue hover:text-brand-darkBlue hover:border-brand-darkBlue">
                      <a href={info.action.href}>{info.action.label}</a>
                    </Button> : <Button asChild variant="outline" className="w-full border-brand-blue text-brand-blue hover:text-brand-darkBlue hover:border-brand-darkBlue">
                      <a href={info.action.href} target={info.action.href.startsWith("http") ? "_blank" : undefined} rel={info.action.href.startsWith("http") ? "noopener noreferrer" : undefined}>
                        {info.action.label}
                      </a>
                    </Button>}
                </div>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Button asChild variant="outline" size="lg" className="border-white text-white hover:bg-white/10">
              <a href="https://wa.me/529671234567" target="_blank" rel="noopener noreferrer">
                WhatsApp
              </a>
            </Button>
          </div>
